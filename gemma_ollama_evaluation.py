#!/usr/bin/env python3
"""
Local Gemma Evaluation using Ollama
Runs Gemma models locally to avoid API rate limits
"""
...
def safe_call_ollama_gemma(model_name: str, prompt: str) -> str:
    """Safely call Gemma model via Ollama API"""
    try:
        # Ollama API endpoint
...
def evaluate_model_ollama(model_name: str, prompt: str) -> Dict[str, Any]:
    """Evaluate a single Gemma model via Ollama"""
    start_time = time.time()
    
    print(f"Testing {model_name} via Ollama...")
    
    # Call the model
    response = safe_call_ollama_gemma(model_name, prompt)
...
def run_ollama_gemma_evaluation():
    """Run evaluation with local Gemma models via Ollama"""
    print("🎯 LOCAL GEMMA EVALUATION VIA OLLAMA")
    print("=" * 45)
    ...
    # Define Gemma models to test via Ollama
    ollama_gemma_models = [
        "gemma:2b",           # Smallest Gemma model
    ...
    for model_name in ollama_gemma_models:
        result = evaluate_model_ollama(model_name, prompt)
    ...
    # Save results
    output_file = os.path.join(os.path.dirname(__file__), "ollama_gemma_results.json")
    with open(output_file, 'w') as f:
        json.dump(results, f, indent=2)
    
    # Print summary
    print(f"\n📊 Ollama Gemma Results Summary:")
    ...
    print("\n🎉 Ollama Gemma evaluation complete!")

if __name__ == "__main__":
    run_ollama_gemma_evaluation()
