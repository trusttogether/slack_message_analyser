#!/usr/bin/env python3
"""
STEP 1: Google Gemini Models Evaluation (Not Gemma)
Tests Gemini models which have better rate limits
"""
...
def safe_call_google_gemini(model_name: str, prompt: str) -> str:
    """Safely call Google Gemini model (not Gemma)"""
    try:
...
def evaluate_model_step1(model_name: str, prompt: str) -> Dict[str, Any]:
    ...
    # Call the model
    response = safe_call_google_gemini(model_name, prompt)
...
def run_gemini_step1_evaluation():
    """Run STEP 1 evaluation with Gemini models"""
    print("🎯 STEP 1: GOOGLE GEMINI MODELS EVALUATION")
    print("=" * 50)
    ...
    # Define Gemini models to test (not Gemma)
    gemini_models = [
        "gemini-1.5-flash",
        "gemini-1.5-pro",
    ...
    print(f"🧪 Testing {len(gemini_models)} Gemini models for STEP 1...")
    ...
    for model_name in gemini_models:
        result = evaluate_model_step1(model_name, prompt)
    ...
    # Save results
    output_file = os.path.join(os.path.dirname(__file__), "gemini_step1_results.json")
    with open(output_file, 'w') as f:
        json.dump(results, f, indent=2)
    
    # Print summary
    print(f"\n📊 Gemini STEP 1 Results Summary:")
    print(f"  Total models tested: {len(gemini_models)}")
    print(f"  Valid responses: {valid_count}")
    print(f"  Success rate: {(valid_count/len(gemini_models)*100):.1f}%")
    ...
    print("\n🎉 Gemini STEP 1 evaluation complete!")

if __name__ == "__main__":
    run_gemini_step1_evaluation()
